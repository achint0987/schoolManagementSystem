<?php
	...
	if(isset($_POST['contact']) && isset($_POST['address']) &&isset($_POST['firstName']) && isset($_POST['lastName']) && isset($_POST['rollNo']) && isset($_POST['email']) && isset($_POST['password']) && isset($_POST['DOJ']) && isset($_POST['branch']) && isset($_POST['semester'])){
			...
			$imageTarget= 	"../images/".basename($_FILES['image']['name']);
			...
			if(mysql_num_rows($runQuery) > 0){
				...
				$query = "INSERT INTO studentdetails (FirstName, LastName, RollNo, Email, Semester, yearOfJoining, Password, Branch, contactNum, address, image)
				 VALUES ('$firstname', '$lastname', '$rollno', '$email', '$semester', '$yoj', '$password', '$branch', '$contact', '$address', '$image')";
